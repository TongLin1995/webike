...
def googemapscoords():
   ...
   stmt = "select stamp, latgps, longgps from imei{0} where stamp >= \"{1}\" and stamp <= \"{2}\" and latgps is not null and longgps is not null order by stamp".format(imei, curdate, end)
   print_err(stmt)
   lats = []
   longs = []
   ...
   lastRow = ""
   lastRowTime = ""
   for l in g.dbc.SQLSelectGenerator(stmt):
       if l is not None:
           if lastRow == "" :
              lastRow = l
              lastRowTime = l[0]
               ...
               if (TIMELAPSE >= 30): #only consider rows 5 seconds apart and where the distance traveled was likely not a GPS error
                   d = haversine(l[1],lastRow[1], l[2], lastRow[2])
                   if d*1000 > 15:
                       lats.append(l[1])
                       longs.append(l[2])
                   lastRow = l
                   lastRowTime = l[0]
   return json.dumps({"lats": lats, "longs": longs})
